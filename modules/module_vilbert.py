...
from modules.until_vilbert import PreTrainedModel
...
class BertBiAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        ...
        x = x.view(*new_x_shape)
        # logger.info("BertBiAttention transpose_for_scores x.shape:{}".format(x.shape))
        return x.permute(0, 2, 1, 3)
    ...
    def forward(
        ...
        # logger.info("input_tensor1:{}".format(input_tensor1.shape))
        # logger.info("input_tensor2:{}".format(input_tensor2.shape))
        # for vision input.
        ...
        key_layer1_t = key_layer1.transpose(-1, -2)
        # logger.info("key_layer1:{}".format(key_layer1.shape))
        # logger.info("key_layer1_t:{}".format(key_layer1_t.shape))
        # logger.info("query_layer2:{}".format(query_layer2.shape))

        # Take the dot product between "query2" and "key1" to get the raw attention scores for value 1.
        attention_scores1 = torch.matmul(query_layer2, key_layer1_t)
        # logger.info("attention_scores1:{}".format(attention_scores1.shape))
        ...
        # logger.info("value_layer1:{}".format(value_layer1.shape))
        context_layer1 = torch.matmul(attention_probs1, value_layer1)
        # logger.info("1context_layer1:{}".format(context_layer1.shape))
        context_layer1 = context_layer1.permute(0, 2, 1, 3).contiguous()
        # logger.info("2context_layer1:{}".format(context_layer1.shape))
        new_context_layer_shape1 = context_layer1.size()[:-2] + (self.all_head_size,)
        context_layer1 = context_layer1.view(*new_context_layer_shape1)
        # logger.info("3context_layer1:{}".format(context_layer1.shape))
...
class BertConnectionLayer(nn.Module):
    ...
    def forward(
        ...
        # logger.info("BertConnectionLayer forward")
        # logger.info("input_tensor1.shpae:{}".format(input_tensor1.shape))
        # logger.info("attention_mask1.shpae:{}".format(attention_mask1.shape))
        # logger.info("input_tensor2.shpae:{}".format(input_tensor2.shape))
        # logger.info("attention_mask2.shpae:{}".format(attention_mask2.shape))
        ...
        # logger.info("bi_output1.shpae:{}".format(bi_output1.shape))
        # logger.info("bi_output2.shpae:{}".format(bi_output2.shape))
        ...
        # logger.info("layer_output1.shpae:{}".format(layer_output1.shape))
        # logger.info("layer_output2.shpae:{}".format(layer_output2.shape))
        # logger.info("co_attention_probs:{}".format(co_attention_probs))
...
class co_attention_model(nn.Module):
    def __init__(self):
        ...
        if use_pretrain:
            ...
            self.c_layer = model.bert.encoder.c_layer
        else:
            co_attention_layer = BertConnectionLayer(co_config)
            self.c_layer = nn.ModuleList(
                [copy.deepcopy(co_attention_layer) for _ in range(self.num_co_attention_layer)]
            )

    def forward(self,input_text,input_video):
        for i_layer in range(self.num_co_attention_layer):
            input_video, input_text, _ = self.c_layer[i_layer](input_video, input_text)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        layer = BertLayer(config) #bert的自注意力层
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(input_txt, attention_mask, task_ids)
